...
@router.get("/", summary="Получить всех администраторов", tags=['Администраторы'], response_model=List[AdminResponse])
...
@router.post("/", summary="Добавить администратора", tags=['Администраторы'])
async def create_admin(admin_data: AdminCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Admin).where(Admin.telegram_user_id == admin_data.telegram_user_id)
    )
...
@router.patch("/{admin_id}/toggle", summary="Активировать/деактивировать администратора", tags=['Администраторы'])
